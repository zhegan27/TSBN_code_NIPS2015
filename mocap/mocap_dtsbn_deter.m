...
initialParameters{5}=.001*randn(M,K); % W5prime
initialParameters{6}=.001*randn(K,M,nt); % W6
initialParameters{7}=.001*randn(M,M,nt); % W7
initialParameters{8}=.001*randn(M,M,nt); % W7prime

initialParameters{9}=.001*randn(J,J,nt); % U1
initialParameters{10}=.001*randn(J,K); % U2
...
initialParameters{17}=zeros(M,1); % b3prime

initialParameters{18}=zeros(J,1); % c1
initialParameters{19}=zeros(K,1); % c2

L = 100;
initialParameters{20}=.001*randn(1,L); % A1
initialParameters{21}=.001*randn(L,M); % A2
...
%%
[param,result] = mocap_dtsbn_deter_ascent(TrainData,initialParameters,opts,TestData);

% save('mocap_dtsbn_deter.mat','param', 'result');
