function [parameters,result]= mocap_tsbn_ascent(v,parameters,opts,vtest)
...
for iter=1:opts.iters
    ...
    index = randperm(N);
    vB = v{index(1)}';
    [grads,~,meanll,varll] = mocap_tsbn_gradient(vB,parameters,prevMean,prevVar);
...
%         for i = 1:Ntest
%             vB = vtest{i}';
%             tmp1 = tmp1 + mocap_tsbn_calc_loglike(vB,parameters)/Ntest;
%             [tt2,tt3] = mocap_tsbn_calc_pred_error(vB,parameters);
%             tmp2 = tmp2 + tt2/Ntest;
%             tmp3 = tmp3 + tt3/Ntest;
%         end;

        vB = vtest{1}';
        tmp1 = mocap_tsbn_calc_loglike(vB,parameters);
        [tmp2,tmp3] = mocap_tsbn_calc_pred_error(vB,parameters);
