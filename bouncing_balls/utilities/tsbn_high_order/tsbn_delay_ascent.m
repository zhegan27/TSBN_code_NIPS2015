function [parameters,result]=tsbn_delay_ascent(v,parameters,opts,vtest)
...
for iter=1:opts.iters    
    ...
    index = randperm(N);
    vB = v{index(1)}';
    [grads,~,meanll,varll] = tsbn_delay_gradient(vB,parameters,prevMean,prevVar);
    ...
    if mod(iter,opts.evalInterval)==0
        ...
        vB = vtest{index(1)}';
        tmp1 = tsbn_delay_calc_loglike(vB,parameters);
        [tmp2,tmp3] = tsbn_delay_calc_pred_error(vB,parameters);
