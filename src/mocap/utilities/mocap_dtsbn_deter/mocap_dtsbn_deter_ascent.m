function [parameters,result] = mocap_dtsbn_deter_ascent(v,parameters,opts,vtest)
...
for iter=1:opts.iters
    ...
    index = randperm(N);
    vB = v{index(1)}';
    [grads,~,meanll,varll] = mocap_dtsbn_deter_gradient(vB,parameters,prevMean,prevVar);
    ...
    if mod(iter,opts.evalInterval)==0
        vB = vtest{1}';
        tmp1 = mocap_dtsbn_deter_calc_loglike(vB,parameters);
        [tmp2,tmp3] = mocap_dtsbn_deter_calc_pred_error(vB,parameters);
