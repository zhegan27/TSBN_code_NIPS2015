function [parameters,result]=ar_delay_ascent(v,parameters,opts,vtest)
...
for iter=1:opts.iters    
    ...
    index = randperm(N);
    vB = v{index(1)}';
    grads = ar_delay_gradient(vB,parameters);
    ...
    if mod(iter,opts.evalInterval)==0
        ...
        vB = vtest{index(1)}';
        tmp1 = ar_delay_calc_loglike(vB,parameters);
        tmp2 = ar_delay_calc_pred_error(vB,parameters);
        
        logpv_test=[logpv_test;tmp1]; totaltime = toc;
        predErr_test = [predErr_test;tmp2]; 
