function [parameters,result]=hmsbn_ascent(v,parameters,opts,vtest)
...
for iter=1:opts.iters
    ...
    index = randperm(N);
    vB = v{index(1)}';
    [grads,~,meanll,varll] = hmsbn_gradient(vB,parameters,prevMean,prevVar);
    ...
    if mod(iter,opts.evalInterval)==0
        ...
        for i = 1:Ntest
            vB = vtest{i}';
            tmp1 = tmp1 + hmsbn_calc_loglike(vB,parameters)/Ntest;
            [tt2,tt3] = hmsbn_calc_pred_error(vB,parameters);
            tmp2 = tmp2 + tt2/Ntest;
            tmp3 = tmp3 + tt3/Ntest;
        end;                
        logpv_test=[logpv_test;tmp1]; recErr_test = [recErr_test;tmp2];
        predErr_test = [predErr_test;tmp3]; totaltime = toc;
