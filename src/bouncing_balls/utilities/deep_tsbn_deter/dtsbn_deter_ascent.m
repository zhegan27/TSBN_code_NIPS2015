function [parameters,result]=dtsbn_deter_ascent(v,parameters,opts,vtest)
...
%     end;
    ...
    index = randperm(N);
    vB = v{index(1)}';
    [grads,~,meanll,varll] = dtsbn_deter_gradient(vB,parameters,prevMean,prevVar);
    ...
    if mod(iter,opts.evalInterval)==0
        ...
        vB = vtest{index(1)}';
        tmp1 = dtsbn_deter_calc_loglike(vB,parameters);
        [tmp2,tmp3] = dtsbn_deter_calc_pred_error(vB,parameters);
